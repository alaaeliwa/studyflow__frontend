...
const ACADEMIC_PERIOD_REGEX = /^[\p{L}\p{N}\s./-]+$/u;
...
export function AddCourseDialog({
  ...
  const selectedSemesterForStatus = useMemo(
    () => selectableSemesters.find((s) => s.id === (semesterId || formData.semesterId)),
    [formData.semesterId, selectableSemesters, semesterId],
  );
  const isCompletedSemester = selectedSemesterForStatus?.status === "completed";
  ...
  const validateForm = () => {
    ...
    const academicPeriod = formData.academicPeriod?.trim() || "";
    const numericGrade = formData.numericGrade;

    if (!formData.title.trim()) newErrors.title = "Course name is required";
    if (isNaN(formData.credits) || formData.credits < 1) {
      newErrors.credits = "Credit hours are required";
    } else if (formData.credits > MAX_COURSE_CREDITS) {
      newErrors.credits = `Credits must be between 1 and ${MAX_COURSE_CREDITS}`;
    }
    ...
    if (isCompletedSemester && formData.status === "planned") {
      newErrors.status = "You cannot add a planned course to a completed semester";
    }
    ...
    if (isPriorMode) {
      if (!academicPeriod) {
        newErrors.academicPeriod = "Taken year/semester is required";
      } else if (!ACADEMIC_PERIOD_REGEX.test(academicPeriod)) {
        newErrors.academicPeriod = "Use only letters, numbers, spaces, /, -, or .";
      }
    }

    if (numericGrade !== undefined && numericGrade !== null && (numericGrade < 0 || numericGrade > 100)) {
      newErrors.numericGrade = "Grade must be between 0 and 100";
    }
                    ...
                    {semesterId === "prior-completed" && (
                        <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                            ...
                            <Input
                                ...
                                maxLength={100}
                                className={cn(
                                  "border-blue-200 focus-visible:ring-blue-200",
                                  errors.academicPeriod && "border-red-500"
                                )}
                            />
                            {errors.academicPeriod && <p className="text-sm text-red-500">{errors.academicPeriod}</p>}
                        </div>
                    )}
                            ...
                            <Select 
                                ...
                                onValueChange={(val) => {
                                  ...
                                  setFormData({
                                    ...
                                    semesterId: val,
                                    status:
                                      selectedSemester?.status === "completed" && formData.status === "planned"
                                        ? "current"
                                        : formData.status,
                    ...
                    {semesterId !== "prior-completed" && (
                        <div className="space-y-2">
                            ...
                            <CourseStatusSelector
                                ...
                                error={errors.status}
                                disabledOptions={isCompletedSemester ? ["planned"] : []}
                            />
                            {isCompletedSemester && (
                                <p className="text-xs text-amber-600">
                                    Planned courses are not allowed inside a completed semester.
                                </p>
                            )}
                        </div>
                    )}
