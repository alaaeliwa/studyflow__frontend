...
export function useAppState() {
        ...
        if (savedUser) {
          try {
            ...
            AppStore.update({
              ...
              streak: parsed.streak || EMPTY_APP_STATE.streak
            });
          } catch {
            // ignore parse errors
          }
  ...
  const saveUnifiedTask = useCallback(async (task: TaskItem) => {
    ...
    if (existingTask) {
      AppStore.update(prev => keepStreakAlive({
        ...prev,
        tasks: prev.tasks.map(t => String(t.id) === String(task.id) ? task : t),
      }));
      // تعديل task موجودة
      try {
        const saved = await DataService.updateTask(task);
        ...
        AppStore.update(prev => keepStreakAlive({ ...prev, tasks: prev.tasks.map(t => String(t.id) === String(existingTask.id) ? existingTask : t) }));
      }
    } else {
      // إضافة task جديدة
      try {
