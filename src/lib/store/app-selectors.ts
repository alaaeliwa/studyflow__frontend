...
export function selectCalendarEvents(state: AppState): CalendarEvent[] {
  ...
  const inferExamEventType = (value?: string): CalendarEvent["type"] => {
  ...
  const mapTaskTypeToCalendarType = (task: TaskItem): CalendarEvent["type"] => {
    switch (task.type) {
      case "assignment":
        return "assignment";
      case "quiz":
        return "quiz";
      case "exam":
        return inferExamEventType(task.title);
      default:
        return "task";
    }
  };
  ...
  state.tasks.forEach((task) => {
    if (task.dueDate) {
      events.push({
        ...
        type: mapTaskTypeToCalendarType(task),
    ...
    course.exams?.forEach((e) => {
      if (e.date) {
        events.push({
          ...
          date: e.date,
          time: e.time,
          type: inferExamEventType(`${e.title} ${e.type ?? ""}`),
          status: "upcoming",
          courseId: course.id,
          courseCode: course.code,
          courseName: course.title,
